...
def find_optimal_threshold(
    ...
    return best_threshold, thresholds, f1_scores[:-1], best_f1, auprc
...
if __name__ == "__main__":
    ...
    mlp_model = load_mlp(x_val)

    dt_val_probs = dt_model.predict_proba(x_val)[:, 1]
    rf_val_probs = rf_model.predict_proba(x_val)[:, 1]

    with torch.no_grad():
        logits = mlp_model(torch.tensor(x_val.values, dtype=torch.float32))
        mlp_val_probs = torch.sigmoid(logits).cpu().numpy().ravel()

    # 3. Find Optimal Thresholds
    dt_thresh, dt_ts, dt_f1s, dt_best_f1, dt_auprc = find_optimal_threshold(
        y_val,
        dt_val_probs,
        model_name="Decision Tree",
    )
    rf_thresh, rf_ts, rf_f1s, rf_best_f1, rf_auprc = find_optimal_threshold(
        y_val,
        rf_val_probs,
        model_name="Random Forest",
    )
    mlp_thresh, mlp_ts, mlp_f1s, mlp_best_f1, mlp_auprc = find_optimal_threshold(
        y_val,
        mlp_val_probs,
    ...
    thresholds = {
        ...
        "dt_validation_f1": dt_best_f1,
        "rf_validation_f1": rf_best_f1,
        "mlp_validation_f1": mlp_best_f1,
        "dt_validation_auprc": dt_auprc,
        "rf_validation_auprc": rf_auprc,
        "mlp_validation_auprc": mlp_auprc,
        "tuned_on": "validation",
    }
    ...
    plt.subplot(1, 2, 2)
    for name, probs in [("DT", dt_val_probs), ("RF", rf_val_probs), ("MLP", mlp_val_probs)]:
